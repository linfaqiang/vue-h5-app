    ...
    <div class="mui-content">
      ...
      <div v-show="chance.dataList.length>0">
        <mui-scroll-refresh bottom="0" top="84px">
          <div class="rival_li">
            <ul>
              <li v-for="list in chance.dataList" class="mui-icon" :class="{selected: list.selected, 'crm-check': list.selected}" @tap="chanceSelect($index)">
  ...
  export default {
    ...
    methods: {
      ...
      chanceSelect: function (index) {
        ...
        if (arrPram[1] === 'add') {
          store.updateField('chance', backData.chanceName, 'add');
          store.updateField('chanceId', backData.id, 'add');
          store.updateField('customer', backData.customerName, 'add');
          store.updateField('customerId', backData.customerId, 'add');
