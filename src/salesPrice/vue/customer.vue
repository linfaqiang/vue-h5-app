    ...
    <div class="mui-content">
      ...
      <div v-show="customer.dataList.length>0">
        <mui-scroll-refresh bottom="0" top="84px">
          <div class="rival_li">
            <ul>
              <li v-for="list in customer.dataList" class="mui-icon" :class="{selected: list.selected, 'crm-check': list.selected}" @tap="customerSelect($index)">
    ...
    methods: {
      ...
      customerSelect: function (index) {
        ...
        if (arrPram[1] === 'add') {
          ...
          store.updateField('chance', '', arrPram[1]);
          store.updateField('chanceId', '', arrPram[1]);
